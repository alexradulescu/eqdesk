...
test("RevenueCat rollout meters signed-in readers and preserves purchased access across rollback", async () => {
  const visitor = reader();
  const form = (fields) => ({
    method: "POST",
  ...
  await visitor.request("/auth/callback?user=auth0%7Calison");
  await visitor.request("/demo/revenuecat/reset", { method: "POST" });
  try {
    assert.equal(
    ...
    const checkout = await (
      await visitor.request(
        `/demo/revenuecat/checkout?returnTo=${encodeURIComponent(article)}`,
      )
    ).text();
    ...
    await visitor.request("/auth/callback?user=auth0%7Calison");
    for (const experience of ["legacy_regwall", "revenuecat"]) {
      await visitor.request(
        "/devtools/access-experience",
        form({ experience }),
      );
...
test("anonymous checkout login can be cancelled back to the article", async () => {
  ...
  const checkout = `/demo/revenuecat/checkout?returnTo=${encodeURIComponent(article)}`;
